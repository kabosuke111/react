...
const App = () => {
  ...
  const icon_spring = useSpring({
    config: config.gentle,
    transform: `scale(${anims.Big}, ${anims.Big}) rotate(${anims.A_rotate*2}deg)`,
  });
  ...
  return (
    ...
    //マウスが要素に重なったらステートのopacity変更
    //map関数により順番に要素をセット
      <header id="header" onMouseLeave={()=>animsAct({
        ...anims, A_top:100, A_rotate: 180, A_width: 0, Big: 1})} >
        <animated.h1 className="top_icon" style={icon_spring} onMouseEnter={()=>animsAct({
          ...anims, A_top:0, A_rotate:0, A_width:100, Big: 1.2})}><animated.img src={TopIcon} alt="breeze group" />
        </animated.h1>
